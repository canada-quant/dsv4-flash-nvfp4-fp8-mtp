...
def build_nvfp4_recipe(dry_run_one_layer: bool):
    ...
    return QuantizationModifier(
        ...
        ignore=[
            ...
            # V2 NARROWER MTP IGNORE (2026-05-21): include MTP attn + FFN
            # experts in quantization scope so the MTP draft model loads
            # cleanly in vLLM mainline (which assumes FP8 wo_a for the
            # fp8_einsum attention forward path). The previous broad
            # `re:.*mtp\..*` ignore left MTP entirely BF16, which caused
            # an `AttributeError: ColumnParallelLinear has no attribute
            # weight_scale` at attention_impl when vLLM's drafter loaded
            # the MTP block (it expects FP8 wo_a scales).
            #
            # We still exclude the modules that triggered the original
            # inference-mode crash at qparam writeback (#2745):
            #   - mtp.0.embed.* (tied to main embed_tokens — shared lookup)
            #   - mtp.0.e_proj.* (input projection reading from embed)
            #   - mtp.0.h_proj.* (hidden projection)
            # MTP norms (enorm/hnorm/eh_norm) are already covered by the
            # broader `re:.*norm.*` ignore above.
            #
            # If MTP attn or experts writeback ALSO triggers inference-mode
            # crash (because their forward path depends on the shared embed
            # output via residuals), this re-calibration will fail at
            # subgraph 44 and we'll need to fall back to Option B (patch
            # vLLM attention.py:334/508 for BF16 wo_a path).
            r"re:.*mtp\.\d+\.embed.*",
            r"re:.*mtp\.\d+\.e_proj.*",
            r"re:.*mtp\.\d+\.h_proj.*",
        ],
